...
let questionToTheUser = prompt("Сколько дроидов вы хотите купить?");
let message;
let totalPrice;

if (questionToTheUser === null) {
  message = "Отменено пользователем!";
  console.log(message);
} else if (questionToTheUser > 0) {
  totalPrice = pricePerDroid * questionToTheUser;
  if (totalPrice > credits) {
    message = "Недостаточно средств на счету!";
    console.log(message);
  } else {
    credits = credits - totalPrice;
    message = `Вы купили ${questionToTheUser} дроидов, на счету осталось ${credits} кредитов.`;
    console.log(message);
  }
